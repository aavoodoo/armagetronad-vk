...
tPlatformNull::~tPlatformNull() = default;

tPlatformCaps tPlatformNull::GetCapabilities() const
{
    tPlatformCaps caps;
    caps.hasHighResTimer = IsTimerAccurate();
    caps.isTimerMonotonic = timerIsMonotonic_;
    caps.hasGraphics = false; // Dedicated server has no graphics
    return caps;
}

void tPlatformNull::GetTimeInternal(tPlatformTime& time) const
{
#ifdef WIN32
    if (hpcReliable_)
    {
...
void tPlatformNull::GetTime(tPlatformTime& time) const
{
...
void tPlatformNull::Sleep(int microseconds) const
{
    if (microseconds <= 0)
        return;
...
bool tPlatformNull::IsTimerAccurate() const
{
#ifdef WIN32
    return hpcReliable_;
#else
...
bool tPlatformNull::IsTimerMonotonic() const
{
